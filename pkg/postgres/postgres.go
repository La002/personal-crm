...
func ConnectDB(config *config.Configuration, l *logger.Logger) *gorm.DB {
	...
	// Check for DB_URL (from environment or config)
	if config.DB.URL != "" {
		l.Info("Using DB_URL from environment/config")
		connectStr = config.DB.URL
	} else {
		// Fallback to individual config fields (local development)
		l.Info("Using individual DB config fields")
